<template>
    <div class="layout-padding layout-padding-unset layout-iframe">
        <div class="layout-padding-auto layout-padding-view">
            <div v-for="v in setIframeList" :key="v.path" v-loading="!!v.meta?.loading" class="w100"
                element-loading-background="white">
                <transition-group :name="name">
                    <iframe v-show="getRoutePath === v.path" :key="v.path" ref="iframeRef" :src="v.meta?.isLink"
                        frameborder="0" height="100%" width="100%" style="position: absolute" :data-url="v.path" />
                </transition-group>
            </div>
        </div>
    </div>
...
const setIframeList = computed(() => {
    return props.list.filter((v: RouteItem) => v.meta?.isIframeOpen)
})
// 获取 iframe 当前路由 path
